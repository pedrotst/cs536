...
void sigchld_handler(int s){
  // waitpid might ovveride errno, so we save it to restore later
...
int main(void){
  ...
  while(1) { //main accept() loop
    ...
    if(!fork()) { // this is the child process
      ...
      splits[0] = strtok(buf, " ");
      int i = 0;
      while(splits[i] != NULL && i < 10){
        i++;
        splits[i] = strtok(NULL, " ");
      }

      // Now we can call execvp
      if(execvp(splits[0], splits) == -1){	// if execution failed, terminate child
        printf("[Server]: Error!\n");
        close(new_fd);
        exit(1);
      }
