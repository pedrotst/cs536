...
KB and target buffer occuppancy of 24 KB.
...
./playaudio 128.10.2.13 3344 /tmp/$USER/$FILE.au 1488 3 49152 24576 $FILE_receiver_$TMODE.txt
\end{lstlisting}

where \lstinline{$FILE} is the file being transfered, and \lstinline{$TMODE} is
the transfer method A, B, C or D. All of these generated log files can be found
in \texttt{lab6/v1/logs}.

We then run each mode for the provided files \texttt{pp.au} and \texttt{klyne-jarrett.au},
and plot all of these runs using gnuplot.

The data plot for the transfer of the file pp on the sender side are
for methods A, B, C, and D are, respectivelly, the following:
...
For the big file \texttt{klyne-jarrett} we have the plots on the same style.
