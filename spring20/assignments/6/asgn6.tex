...
\section*{Problem 3}
First the host needs to get an IP address by sending a DHCP broadcast and the DHCP server will respond with DHCP ACK message, containing an allocated IP address for the host use, the IP address of the DNS server, the IP address of the default gateway router R, and a network mask. The host then contact the DNS server by sending its message to R. The DNS server responds with the IP address C the host wishes to download the page from. With C address, the host can finally create a TCP socket. After the three-way handshake the host can send an HTTP GET, C responds with an HTTP response message, places the requested webpage in the body of the message and sends it through the socket. When the message is finally received by the host it can extract the Web page and finally display it to the user.
